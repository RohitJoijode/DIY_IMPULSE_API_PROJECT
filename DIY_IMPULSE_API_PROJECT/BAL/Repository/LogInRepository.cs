...
namespace DIY_IMPULSE_API_PROJECT.BAL.Repository
{
    public class LogInRepository : ILogInRepository
    {
        ...
        public LogInRepository(IConfiguration config, DBEngine DbEngine)
        {
            _config = config;
            _DbEngine = DbEngine;
        }
        ...
        {
            DAL.Tbl_Users Tbl_Users = null;

            using (var transaction = _DbEngine.Database.BeginTransaction())
            {
                var UserExits = _DbEngine.Tbl_Users.Where(x => x.UserName == user.UserName
                ...
                if (UserExits == null)
                {
                    Tbl_Users = new DAL.Tbl_Users();
                    var maxUserId = await _DbEngine.Tbl_Users.MaxAsync(u => (int?)u.UserId) ?? 0;
                    ...
                    _DbEngine.Tbl_Users.Add(Tbl_Users);
        ...
        public async Task<LoginResponse> Login(LogInUser user)
        {
            ...
            try
            {
                ...
                if (UserExits is null || !(await _DbEngine.Tbl_Users
